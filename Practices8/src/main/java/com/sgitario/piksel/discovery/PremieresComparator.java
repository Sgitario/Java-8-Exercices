...
public class PremieresComparator {
	...
	public PremieresComparator() {
		...
		causes.add(new LangStatusNotNullCause());
	}
	...
	class FirstNodeTenDaysCause implements Cause {
		...
		public boolean isCausedBy(Premiere prev, Premiere after) {
			try {
				if (prev.langStatus.isEmpty() 
					...
					if (vendorDateLess(after, 10)) {
						return true;
					} else if (vendorDateLess(after, 11)) {
						return true;
					} else if (vendorDateLess(after, 9)) {
						return true;
					}
		...
		private boolean vendorDateLess(Premiere after, int days) throws ParseException {
			Calendar vendorDate = Calendar.getInstance();
			vendorDate.setTime(SDF.parse(after.workingTxDate));
			vendorDate.add(Calendar.DAY_OF_YEAR, -days);
